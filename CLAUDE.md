...
```
openai-types/                    ← standalone crate: 1100+ types, auto-synced from Python SDK
  src/
    shared/
      common.rs                  — Role, FinishReason, Usage, ServiceTier (canonical, with Other(String))
      pagination.rs              — ListResponse<T>
      _gen.rs                    — auto-generated shared types
    {domain}/
      _gen.rs                    — auto-generated by py2rust (overwritten on sync)
      manual.rs                  — hand-crafted: enums, builders, Option fields (preserved on sync)
      mod.rs                     — re-exports both
    24 domains: audio, batch, beta, chat, completion, containers, conversations,
                embedding, evals, file, fine_tuning, graders, image, model,
                moderation, realtime, responses, shared, skills, uploads,
                vector_stores, video, webhooks, websocket

src/                             ← HTTP client + resources
  lib.rs                         — pub mod, re-exports
  client.rs                      — OpenAI client (api_key, base_url, retries, HTTP/2 tuning)
  config.rs                      — ClientConfig (timeouts, retries, azure auth)
  streaming.rs                   — SSE stream parser
  pagination.rs                  — Paginator<T> async stream
  resources/                     — one module per API endpoint
  types/                         — pure re-exports from openai-types + openai_enum! macro
    *.rs                         — `pub use openai_types::{domain}::*;` + tests

openai-oxide-node/               ← Node.js native bindings (napi-rs)
  wrapper.js                     — auto fast-path for large payloads (>8KB)
  src/lib.rs                     — Rust→JS bridge

scripts/
  py2rust.py                     — Python SDK → Rust type generator
  migrate_types.py               — migration helper
```
